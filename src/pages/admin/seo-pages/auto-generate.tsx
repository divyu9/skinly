          ...
          try {
            ...
            if (aiContent.success && aiContent.contentHTML) {
              ...
              if (sentence.length <= 160) {
                ...
                // Remove non-essential words while keeping grammar correct
                
                // Strategy: Remove less critical parts in order of priority
                // 1. Remove parenthetical statements
                sentence = sentence.replace(/\s*\([^)]*\)\s*/g, ' ');
                
                // 2. Remove phrases after commas (subordinate clauses) if still too long
                if (sentence.length > 160) {
                ...
                // 3. If still too long, remove prepositional phrases starting with 'for', 'with', 'at', etc.
                if (sentence.length > 160) {
                  sentence = sentence.replace(/\s+(for|with|at|in|on|to|from|by)\s+[^.!?]+[.!?]/, '.');
                }
                
                // 4. Last resort: truncate at word boundary and add period
                if (sentence.length > 160) {
                  let truncated = sentence.substring(0, 157); // Leave room for period and space
              ...
              // Final safety check: ensure it ends with proper punctuation
              ...
              if (!metaDescription || metaDescription.length === 0) {
                metaDescription = `Discover ${item.value} at GoSkinly.`;
              }
