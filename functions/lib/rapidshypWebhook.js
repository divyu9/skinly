...
exports.rapidshypWebhook = void 0;
...
async function expectedToken() {
    var _a;
    if (process.env.RAPIDSHYP_WEBHOOK_TOKEN)
        return process.env.RAPIDSHYP_WEBHOOK_TOKEN;
    const snap = await admin.firestore().collection("settings").doc("rapidshyp_webhook_token").get();
    return snap.exists ? String(((_a = snap.data()) === null || _a === void 0 ? void 0 : _a.value) || "") : "";
}
 ...
 */
function nextStatus(code, shipmentStatus, current) {
...
exports.rapidshypWebhook = (0, https_1.onRequest)(async (req, res) => {
    var _a;
    if (req.method !== "POST") {
    ...
    const raw = typeof req.body === "string" ? req.body : JSON.stringify((_a = req.body) !== null && _a !== void 0 ? _a : {});
    let data;
    try {
    ...
    const shipment = ((record === null || record === void 0 ? void 0 : record.shipment_details) || [])[0];
    if (!record || !shipment) {
    ...
    if (adoptedAwb)
        update.awbNumber = awbNumber;
    if (statusDesc || shipmentStatus)
        update.shippingStatus = statusDesc || shipmentStatus;
    if (target)
    ...
    if (target === "delivered" && !order.walletCreditCredited && order.walletCreditCouponAmount && order.userId) {
        try {
            ...
            console.error("rapidshypWebhook: wallet credit failed", { order: order.orderNumber, error: (e === null || e === void 0 ? void 0 : e.message) || e });
        }
    }
...
async function creditWallet(db, orderRef, order) {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if ((_a = orderSnap.data()) === null || _a === void 0 ? void 0 : _a.walletCreditCredited)
            return;
        if (!userSnap.exists)
            return;
        const before = Number((_b = userSnap.data()) === null || _b === void 0 ? void 0 : _b.walletBalance) || 0;
        const after = before + amount;
        tx.update(userRef, { walletBalance: after });
        tx.set(txRef, Object.assign(Object.assign({ userId: order.userId, transactionType: "credit", amount, source: "coupon_credit", balanceBefore: before, balanceAfter: after, description: `Wallet credit from coupon on order ${order.orderNumber || ""}`.trim(), relatedOrderId: orderRef.id }, (order.couponId ? { relatedCouponId: order.couponId } : {})), { createdAt: Date.now() }));
        tx.update(orderRef, { walletCreditCredited: true });
    });
}
//# sourceMappingURL=rapidshypWebhook.js.map