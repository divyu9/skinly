...
const getKey = () => {
    ...
    if (!key) {
        throw new https_1.HttpsError("failed-precondition", "POYO_API_KEY is not configured on the server");
    }
    return key;
};
...
const poyoRequest = async (path, init, attempt = 0) => {
    let res;
    try {
        res = await fetch(`${POYO_BASE}${path}`, init);
    }
    catch (e) {
        throw new https_1.HttpsError("unavailable", `Could not reach PoYo: ${(e === null || e === void 0 ? void 0 : e.message) || e}`);
    }
    ...
    if (!res.ok) {
        ...
        const detail = asText(body === null || body === void 0 ? void 0 : body.message) || asText(body === null || body === void 0 ? void 0 : body.error) || asText(body === null || body === void 0 ? void 0 : body.data) || text.slice(0, 300);
        ...
        if (/comply|policy|regulation|content.{0,12}(filter|violat)|safety|moderat/i.test(detail)) {
            throw new https_1.HttpsError("invalid-argument", "The model refused this design. Its safety filter reads the reference photo as well as the prompt, " +
        ...
        throw new https_1.HttpsError(res.status === 401 || res.status === 403 ? "permission-denied" : "internal", `PoYo ${res.status}: ${detail || "request failed"}`);
    }
    return body;
};
/**
...
exports.poyoSubmit = (0, https_1.onCall)(async (data, context) => {
    ...
    if (!MODELS.includes(model)) {
        throw new https_1.HttpsError("invalid-argument", `Unsupported model: ${model}`);
    }
    if (size && !SIZES.includes(size)) {
        throw new https_1.HttpsError("invalid-argument", `Unsupported size: ${size}`);
    }
    if (resolution && !RESOLUTIONS.includes(resolution)) {
        throw new https_1.HttpsError("invalid-argument", `Unsupported resolution: ${resolution}`);
    }
    if (quality && !QUALITIES.includes(quality)) {
        throw new https_1.HttpsError("invalid-argument", `Unsupported quality: ${quality}`);
    }
    ...
    if (Array.isArray(imageUrls) && imageUrls.length) {
        if (imageUrls.length > 6) {
            throw new https_1.HttpsError("invalid-argument", "At most 6 reference images");
        }
        for (const u of imageUrls) {
            if (typeof u !== "string" || !/^https:\/\//.test(u)) {
                throw new https_1.HttpsError("invalid-argument", "Reference images must be https URLs");
            }
        }
        input.image_urls = imageUrls;
    }
    else {
        throw new https_1.HttpsError("invalid-argument", `${model} must be called with a reference image; none was supplied`);
    }
    ...
    const taskId = ((_a = body === null || body === void 0 ? void 0 : body.data) === null || _a === void 0 ? void 0 : _a.task_id) || (body === null || body === void 0 ? void 0 : body.task_id);
    if (!taskId) {
