...
export async function fillMissingSeoPages(
  ...
  /*
   * Repair what earlier runs wrote as "device".
   *
   * Those pages resolve to nothing and show the whole catalogue, and there is
   * no way to tell from the outside — so this fixes them rather than waiting
   * for somebody to notice a lens skin on a phone page.
   */
  const wrong = pageSnap.docs.filter((d) => {
    const p = d.data() as any;
    return p.autoGenerated === true && p.pageType === "device";
  });
