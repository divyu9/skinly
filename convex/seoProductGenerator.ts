...
// Generate SEO content from form data (for new products before saving)
export const generateSEOFromFormData = action({
  args: {
    title: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    // Determine product category/type for better context
    const categoryName = args.gadgetCategory ? args.gadgetCategory.replace(/-/g, " ") : "product";
    const finishType = args.finishType ? ` (${args.finishType} finish)` : "";
