...
// ============================================================================
// OPTIMIZED COMBINED QUERIES - Reduces multiple roundtrips to single call
// ============================================================================

/**
 * Get all product detail page data in a single query
 * This replaces 6-8 separate queries with one optimized call
 */
export const getProductDetailPageData = query({
  args: { 
    slug: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    const [
      ...
      product.collectionId 
        ? ctx.db.get(product.collectionId)
        : Promise.resolve(null),
      
      // Get approved reviews (limit 10 for initial load)
      ctx.db
        .query("reviews")
      ...
      // Get cross-sell products (same tags, limit 8)
      (async () => {
    ...
    // 3. Calculate review stats
    const allReviewsForStats = await ctx.db
      .query("reviews")
      .withIndex("by_product_and_status", (q) => 
        q.eq("productId", product._id).eq("status", "approved")
      )
    ...
    const reviewStats = {
      averageRating: allReviewsForStats.length > 0 
        ? allReviewsForStats.reduce((sum, r) => sum + r.rating, 0) / allReviewsForStats.length 
        : 0,
      totalReviews: allReviewsForStats.length,
      ratingDistribution: {
        5: allReviewsForStats.filter(r => r.rating === 5).length,
        4: allReviewsForStats.filter(r => r.rating === 4).length,
        3: allReviewsForStats.filter(r => r.rating === 3).length,
        2: allReviewsForStats.filter(r => r.rating === 2).length,
        1: allReviewsForStats.filter(r => r.rating === 1).length,
      }
    };

    // 4. Get mockup if brand/model provided
    let mockupUrl = null;
    if (args.brand && args.model && variants.length > 0) {
      const firstSku = variants[0].sku;
      if (firstSku) {
        const mockup = await ctx.db
          .query("mockups")
          .withIndex("by_brand_model_sku", (q) => 
            q.eq("brand", args.brand!).eq("model", args.model!).eq("sku", firstSku)
          )
          .first();
        
        if (mockup?.fileId) {
          mockupUrl = await ctx.storage.getUrl(mockup.fileId);
        }
      }
    }
...
/**
 * Get homepage data in a single query
 * Replaces 4-5 separate queries with one optimized call
 */
export const getHomepageInitialData = query({
  args: {},
  handler: async (ctx) => {
    ...
    const [
      ...
      // Homepage settings
      ctx.db.query("homepageSettings").first(),
